...
# can be treated differently depending on the method invoked on them inside 
...
# string_arg_one will not be modified since  string_param_one += "rutabaga"
# returns new string value

#string_arg_two will be modifed since  string_param_two << "rutabaga" 
# mutated the original string

# Why? It seems clear from the above that Ruby treats the two string arguments 
# ently...but not so fast. Actually the arguments are treated in exactly the
# same way...but the results are different.

# In both cases, Ruby passes the arguments "by value", but unlike some other 
# languages, the value that gets passed is a reference to some object.
# The string arguments are passed to the method as references to objects 
# of type String.

# The important distinction is that while a reference is passed, the method
# initializes new local variables for both strings and assigns each reference
# to the new local variables. These are variables that only live within 
# the scope of the method definition.

# So, if both arguments live inside the method as new variables that store
# a reference to the passed objects...why don't the strings behave the same
# way in our output?
...
# The reference to this new object is assigned to string_param_one. 
...
# This means that string_param_one and string_arg_one no longer point 
# to the same object.

# On the other hand, String#<< mutates the calling object. Therefore, 
# string_param_two << "rutabaga" mutates the object referenced by 
# string_param_two, so that it becomes "pumpkinsrutabuga".
# So, because the local variable string_param_two still points to the 
# original object, the local variables string_arg_two and string_param_two 
