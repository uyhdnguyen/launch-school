...
def sorted_word(word)
  ...
  (1...array_of_string.length - 1).each do |counter|

    if array_of_string[counter] =~ /[^0-9a-zA-Z]/
      characters_list[counter] = array_of_string[counter]
    else
      result << array_of_string[counter]
    end

  end

  result.sort!.prepend(array_of_string[0]).append(array_of_string[-1])

  characters_list.each { |index,value| result.insert(index, value) }
...
def scramble_words(string)
  ...
  array_of_string.map {|strings| sorted_word(strings) }.join(' ')
end
